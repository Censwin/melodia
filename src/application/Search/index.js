          ...
          list.map ((item, index) => {
            return (
              <li className="item" key={item.first} onClick={() => setQuery (item.first)}>
                <span className="row_num">{index+1}</span><span>{item.first}</span>
              </li>
            )
          })
        }
    ...
    >
      <Container>
        <div className="search_box_wrapper">
          <SearchBox back={searchBack} newQuery={query} handleSearch={handleSearch}></SearchBox>
        </div>
